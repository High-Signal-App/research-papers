...
export function AuthorsTable({ data }: { data: Row[] }) {
  ...
  React.useEffect(() => {
    if (!drill) return;
    fetch("/data/author_drilldowns.json")
      .then((r) => r.json())
      .then((d) => setDrillData(d[drill] ?? null));
  }, [drill]);
  ...
  const columns: ColumnDef<Row>[] = React.useMemo(
    () => [
      ...
      {
        accessorKey: "sum_citations",
        header: "sum citations",
        cell: ({ getValue }) => (
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Button
            ...
            size="sm"
            className="h-7"
            onClick={() => setDrill(row.original.author)}
          >
            drill
          </Button>
        ),
  ...
  return (
    <>
      <DataTable
        ...
        initialSort={[{ id: "sum_citations", desc: true }]}
        pageSize={25}
      />
      <DrilldownDialog
        open={!!drill}
      ...
      >
        {!drillData ? (
          <div className="text-sm text-muted-foreground">
            No drilldown for this author (only top 50 pre-rendered).
          </div>
        ) : (
          <>
            <DrillPillRow
              label="Top hosts cited across this author's papers:"
              items={drillData.top_hosts || []}
              render={(h) => `${h.host} (${h.n})`}
            />
            <DrillPillRow
              label="Citation-graph communities this author appears in:"
              items={drillData.communities || []}
              render={(c) => `community ${c.community_id} (${c.n})`}
            />
            <DrillRow>
              <div className="text-xs text-muted-foreground mb-2">
                Papers ({drillData.papers.length}):
              </div>
