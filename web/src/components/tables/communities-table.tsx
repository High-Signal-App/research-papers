...
export function CommunitiesTable({ data }: { data: Row[] }) {
  ...
  React.useEffect(() => {
    ...
    fetch("/data/community_drilldowns.json")
      .then((r) => r.json())
      .then((d) => setDrillData(d[String(drill)] ?? null));
  }, [drill]);
  ...
  const columns: ColumnDef<Row>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <a
            href={`https://arxiv.org/abs/${row.original.anchor_arxiv_id}`}
            target="_blank"
            rel="noopener"
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Button
            ...
            size="sm"
            className="h-7"
            onClick={() => setDrill(row.original.id)}
          >
            drill
          </Button>
        ),
      ...
      >
        {!drillData ? (
          <div className="text-sm text-muted-foreground">
            No drilldown for this community (only top 30 pre-rendered).
          </div>
        ) : (
          <>
            <DrillPillRow
              label="Top hosts cited by this community:"
              items={drillData.top_hosts || []}
              render={(h) => `${h.host} (${h.n})`}
            />
            <DrillPillRow
              label="Most prolific authors:"
              items={drillData.top_authors || []}
              render={(a) => `${a.author} (${a.n})`}
            />
            <DrillRow>
              <div className="text-xs text-muted-foreground">
                Year range: {drillData.years?.[0]?.year}–
                {drillData.years?.[drillData.years.length - 1]?.year} (
                {drillData.years?.length || 0} years active)
              </div>
            </DrillRow>
            <DrillRow>
