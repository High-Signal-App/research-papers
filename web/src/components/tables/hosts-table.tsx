...
export function HostsTable({ data }: { data: Row[] }) {
  ...
  React.useEffect(() => {
    if (!drill) return;
    fetch("/data/host_drilldowns.json")
      .then((r) => r.json())
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Button
            ...
            size="sm"
            className="h-7"
            onClick={() => setDrill(row.original.host)}
          >
            drill
          </Button>
        ),
  ...
  return (
    <>
      ...
      <DrilldownDialog
        ...
        title={`Papers citing ${drill ?? ""}`}
      >
        {!drillData ? (
          <div className="text-sm text-muted-foreground">
            No drilldown for this host (only top 30 pre-rendered).
          </div>
        ) : (
          drillData.citations.map((c: any, i: number) => (
            <DrillRow key={i}>
              <a
                href={c.url_canonical}
                target="_blank"
                rel="noopener"
              ...
              <div className="text-xs text-muted-foreground mt-0.5">
                <a
                  href={`https://arxiv.org/abs/${c.citing_arxiv_id}`}
                  target="_blank"
                  rel="noopener"
              ...
              {c.context && (
                <div className="text-xs text-muted-foreground/80 italic mt-1">"…{c.context}…"</div>
              )}
            </DrillRow>
          ))
        )}
